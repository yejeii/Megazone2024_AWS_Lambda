import APIHandler from "./src/APIHandler.jsx";
const API = new APIHandler();
import React, { useState, useEffect, useRef } from 'react';

// 카드 클래스
const Card = ({ card, onCardUpdate, onDelete }) => {
  const [title, setTitle] = useState(card.title || '');

  const handleChange = (e) => {
    setTitle(e.target.value);
    if (e.target.value.trim()) {
      onCardUpdate({ ...card, title: e.target.value });
    }
  };

  return (
      <div className="card" draggable onDragStart={card.ondragstart} onDragEnd={card.ondragend}>
